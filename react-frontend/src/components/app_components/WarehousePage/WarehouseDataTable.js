...
const WarehouseDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate1 = (rowData, { rowIndex }) => <p >{rowData.user?.name}</p>
...
<Column field="user" header="User" body={dropdownTemplate1} filter={selectedFilterFields.includes("user")} hidden={selectedHideFields?.includes("user")}  style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Warehouse Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search Warehouse" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
