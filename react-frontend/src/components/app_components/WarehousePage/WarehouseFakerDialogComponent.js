...
import warehouseFakerFactory from "./warehouseFakerFactory";
...
const WarehouseFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = warehouseFakerFactory(props.user,fakerCount,userIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("warehouse").create(elem));
...
export default connect(mapState, mapDispatch)(WarehouseFakerDialogComponent);
